...
<body>
    ...
    <div id="container">
        ...
        <div id="detailArea">
            <p><font style="font-size: 20px">物品类别：</font>${lost.losttype.typename}</p>
            <p><font style="font-size: 20px">物品信息：</font>${lost.lostname}</p>
            <p><font style="font-size: 20px">具体情况：</font><textarea style="width: 310px;height: 100px">${lost.lostdesc}</textarea></p>
            <p><font style="font-size: 20px">物品现在状态：</font>${lost.loststatus.statusdesc}</p>
            <p><font style="font-size: 20px">丢失大致时间：</font><fmt:formatDate value="${lost.losttime}" pattern="yyyy-MM-dd" /></p>
        </div>

        <div id="userArea">
            <p><font style="font-size: 20px">失主姓名：</font>${lost.lostuser.username}</p>
            <p><font style="font-size: 20px">失主性别：</font>${lost.lostuser.usergender}</p>
            <p><font style="font-size: 20px">失主联系方式：</font>${lost.lostuser.usertel}</p>
            <p><font style="font-size: 20px">失主所在年级：</font>${lost.lostuser.usergrade.gradename}</p>
            <p><font style="font-size: 20px">失主所在校区：</font>${lost.lostuser.userlocation.locationname}</p>

            <c:if test="${lost.lostuser == user && lost.loststatus.statusid == 2}">
                <div><a href="/lost/updateLostStatus?lostid=${lostid}">已经找到了</a></div>
            </c:if>
        </div>
