...
<html>
  ...
  <div id="content">
    ...
    <div id="content-body">
      <div class="myRow-right">
        <c:if test="${finds != null}">
          <table>
            <tr>
              <c:forEach items="${finds}" var="find" begin="0" end="3">
                <td>
                    <%-- 每项数据点击可以跳转到StoreServlet 通过cid获取商品信息展示到 商品详情页面 --%>
                  <a href="/find/getFindById/${find.findid}" target="_top">
                    <img src="/image/find/huaweiP30.jpg"/>
                    <div style="display: inline-block;position: relative;left: 4px;top: -80px">
                      <span style="font-size: 25px">失物招领：</span><br>
                      <span>${find.findname}</span><br>
                    </div>
                    <p>物品类别：${find.findtype.typename}</p>
                    <p>具体情况：${find.finddesc}</p>
            ...
            <tr>
              <c:forEach items="${finds}" var="find" begin="4" end="7">
                <td>
                    <%-- 每项数据点击可以跳转到StoreServlet 通过cid获取商品信息展示到 商品详情页面 --%>
                  <a href="/find/getFindById/${find.findid}">
                    <img src="/image/find/huaweiP30.jpg"/>
                    <div style="display: inline-block;position: relative;left: 4px;top: -80px">
                      <span style="font-size: 25px">失物招领：</span><br>
                      <span>${find.findname}</span><br>
    ...
    <div id="bottom" >
      <a href="/find/getFindsByPage" style="display:inline-block;position:absolute;right: 20px">查看更多信息</a>
    </div>
  </div>
  </body>
  <script>
